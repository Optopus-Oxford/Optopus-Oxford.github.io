...
nav: true
nav_order: 2
description: Alternative table view for the Optopus schedule.
---

{% assign schedule_terms = site.data.reading_group_schedule | map: "term" | uniq %}

<div class="schedule-subscribe" aria-label="Calendar subscriptions">
  {% for term in schedule_terms %}
    {% assign calendar_slug = term | downcase %}
    {% assign calendar_url = '/calendars/' | append: calendar_slug | append: '.ics' | relative_url %}
    <div class="calendar-link">
      <a href="{{ calendar_url }}">{{ term }} calendar</a>
...
<div class="schedule-controls" aria-label="Schedule filters">
  <label>
    <span>Term</span>
    <select id="schedule-2-term-filter">
      {% for term in schedule_terms %}
        <option value="{{ term }}" {% if term == 'TT26' %}selected{% endif %}>{{ term }}</option>
      {% endfor %}
    </select>
  </label>
  ...
  <label>
    <span>Activity</span>
    <select id="schedule-2-kind-filter">
...
<p class="schedule-empty" data-schedule-2-empty-message hidden>No events scheduled for this selection.</p>
    ...
    <tbody>
      {% for event in site.data.reading_group_schedule %}
        <tr
          ...
          data-term="{{ event.term }}"
          data-kind="{{ event.kind }}"
        >
          <td data-label="Week">
            <span class="week-code">{{ event.week_number }}</span>
          </td>
          <td data-label="Date">
            <span class="date-time-line">
              <span class="date-code">{{ event.date | date: "%a %d/%m" }}</span>
...
<script>
  (() => {
    const termFilter = document.getElementById("schedule-2-term-filter");
    const kindFilter = document.getElementById("schedule-2-kind-filter");
    const rows = Array.from(document.querySelectorAll(".schedule-table tbody tr"));
    const emptyMessage = document.querySelector("[data-schedule-2-empty-message]");
